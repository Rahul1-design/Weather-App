  ...
  const fetchWeather = async(city)  => {
    ...
    try{
      ...
      if(err.response && err.response.status === 404){
        ...
        setError('An eror occured. Please try again later.');
      }
      setWeather(null);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <div className='min-h-screen flex flex-col items-center justify-center bg-blue-100 relative overflow-hidden'>
        <div className="w-full h-full absolute top-0 left-0 bg-black/20 z-1"></div>
        <video className='absolute  top-0 left-0 w-full h-full object-cover' autoPlay loop muted>
          <source src={video} type="video/mp4" />
          Your browser doesnt support the video tag!
        </video>
        <div className='z-2 bg-black/70 text-white rounded-lg shadow-lg p-8 max-w-md w-full '>
